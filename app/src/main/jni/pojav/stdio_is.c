...
Java_net_kdt_pojavlaunch_firefly_Logger_begin(JNIEnv *env, __attribute((unused)) jclass clazz, jstring logPath) {
    if(latestlog_fd != -1) {
    ...
    if(logger_onEventLogged == NULL) {
        jclass eventLogListener = (*env)->FindClass(env, "net/kdt/pojavlaunch/firefly/Logger$eventLogListener");
...
JNIEXPORT void JNICALL
Java_net_kdt_pojavlaunch_firefly_Logger_appendToLog(JNIEnv *env, __attribute((unused)) jclass clazz, jstring text) {
...
JNIEXPORT void JNICALL
Java_net_kdt_pojavlaunch_firefly_Logger_setLogListener(JNIEnv *env, __attribute((unused)) jclass clazz, jobject log_listener) {
...
JNIEXPORT void JNICALL
Java_net_kdt_pojavlaunch_firefly_utils_JREUtils_setupExitMethod(JNIEnv *env, jclass clazz,
                                                        jobject context) {
    exitTrap_ctx = (*env)->NewGlobalRef(env,context);
    (*env)->GetJavaVM(env,&exitTrap_jvm);
    exitTrap_exitClass = (*env)->NewGlobalRef(env,(*env)->FindClass(env,"net/kdt/pojavlaunch/firefly/ExitActivity"));
