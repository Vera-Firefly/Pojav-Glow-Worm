...
class SDLGenericMotionListener_API26 extends SDLGenericMotionListener_API24 {
    ...
    boolean setRelativeMouseEnabled(boolean enabled) {
        ...
        if (!SDLActivity.isDeXMode() || Build.VERSION.SDK_INT >= 27 /* Android 8.1 (O_MR1) */) {
            // Route grab state to the launcher grab mechanism instead of
            // capturing pointers on the content view
            mRelativeModeEnabled = enabled;
            org.lwjgl.glfw.CallbackBridge.notifyGrabStateFromSdl(enabled);
            net.kdt.pojavlaunch.firefly.Logger.appendToLog(
                    "SDLBridge: relative mode " + enabled + " forwarded to PGW grab state");
            return true;
        } else {
            return false;
        }
