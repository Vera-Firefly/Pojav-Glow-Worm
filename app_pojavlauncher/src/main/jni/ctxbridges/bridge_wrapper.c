...
bridge_wrapper_t* bridge_init_context(bridge_wrapper_t * share)
{
    ...
    EGLContext ctx = mesa_gl_create_context(share_ctx, 2, &cfg);
    if (ctx == EGL_NO_CONTEXT) {
        LOGE("mesa_gl_create_context failed");
        return NULL;
    }

    bridge_wrapper_t * bundle = (bridge_wrapper_t *) calloc(1, sizeof(bridge_wrapper_t));
    if (!bundle) {
        LOGE("calloc failed");
...
void bridge_make_current(bridge_wrapper_t * bundle)
{
    if (!bundle) {
        ...
        if (eglMakeCurrent_p && g_current_bundle) {
            eglMakeCurrent_p(mesa_gl_create_context(EGL_NO_CONTEXT, 2, NULL) /* noop? */, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
        }
    ...
    LOGI("bridge_make_current: bundle %p is now current (tmp_pb %p)", (void*)bundle, (void*)tmp_pb);
}
...
void bridge_setup_window(void)
{
    ...
    if (g_current_bundle->window && g_current_bundle->window != g_pending_native_window) {
        ANativeWindow_release(g_current_bundle->window);
        g_current_bundle->window = NULL;
    }

    /* Acquire and assign */
    ANativeWindow_acquire(g_pending_native_window);
    g_current_bundle->window = g_pending_native_window;

    /* optionally set buffers geometry based on cfg */
    EGLint native_format = 0;
    if (!g_current_bundle->ctx) {
        g_current_bundle->ctx = mesa_gl_create_context(EGL_NO_CONTEXT, 2, NULL);
    }
    ...
    ANativeWindow_setBuffersGeometry(g_current_bundle->window, 0, 0, native_format);
...
void bridge_swap_buffers(void)
{
    ...
    if (b->ahb_fbo && b->window) {
        /* ensure rendering finished (caller may call glFinish or we call here) */
        glFinish();
        if (mesa_gl_post_fbo_to_window(b->ahb_fbo, b->window) != 0) {
    ...
    if (b->tmp_pbuffer == EGL_NO_SURFACE && b->window) {
